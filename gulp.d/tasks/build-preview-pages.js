...
function copyImages(src, dest) {
	...
	const streamPreview = vfs
		.src(srcGlob, { base: src, cwd: src })
		.pipe(vfs.dest(dest));
	// Write staging copies under dest/_/img to ensure images are nested under public/_/img in the bundle
	const streamStaging = vfs
		.src(srcGlob, { base: src, cwd: src })
		.pipe(vfs.dest(path.join(dest, "_", "img")));
	return merge(streamPreview, streamStaging).pipe(
		map((file, enc, next) => next()),
	);
}
