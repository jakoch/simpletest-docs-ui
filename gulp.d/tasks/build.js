...
// gulp-imagemin is an ESM module that uses top-level await. Load it dynamically
// at runtime so it isn't required from a CommonJS module.
...
module.exports = (src, dest, preview) => {
	// Helper that constructs the stream given an imagemin plugin (or a
	// passthrough). This function is synchronous and returns a vinyl stream.
	const makeStream = (imageminPlugin) => {
		...
		return merge(
			...
			vfs
				...
				.pipe(
					map((file, enc, next) =>
						next(
							null,
							Object.assign(file, { extname: "" }, { extname: ".js" }),
						),
					),
				),
			...
			vfs
				.src(["css/site.css", "css/vendor/*.css"], {
					...opts,
					sourcemaps,
					allowEmpty: true,
				})
	...
	if (preview) {
		// Return a synchronous function that returns a stream immediately for incremental preview builds
		return () => makeStream(through());
	}

	// For full builds, return an async function so we can dynamically import the ESM imagemin package
	return async () => {
		if (!imageminModule) imageminModule = await import("gulp-imagemin");
		const im = imageminModule;
		const plugins = [
			im.gifsicle?.(),
			im.mozjpeg?.(),
			im.optipng?.(),
		...
		const imageminPlugin = (im.default || im)(plugins);
		return makeStream(imageminPlugin);
	};
};
